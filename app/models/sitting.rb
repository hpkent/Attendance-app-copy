class Sitting < ActiveRecord::Base
  ...
  def sitting_info 
    "#{self.event_title} - #{self.start_date.strftime("%b-%d-%Y")}" 
  end
  ...
  def self.update_calendar(items)
    ...
    items.each do |sitting|
      ...
      if (local_start_time!=nil) then
        @calendar_sitting = Sitting.where(start_time:local_start_time).first_or_initialize  do |sitting_record|
            sitting_record.event_title = summary
